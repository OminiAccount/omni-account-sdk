...
async function buildPoseidon() {
    ...
    const F = new ffjavascript_1.F1Field(goldenPrime);
    const t = 12;
    const nRoundsF = 8;
    const nRoundsP = 22;
    const C = [
    ...
    const poseidonHash = (inputs, capacity) => {
        if (inputs.length !== 8)
            throw new Error('Invalid Input size (must be 8)');
        let state;
        if (capacity) {
