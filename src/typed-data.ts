...
export class TypedDataEncoder {
   ...
   */
  async getEncoder(type: string): Promise<(value: any) => string> {
  ...
  async #getEncoder(type: string): Promise<(value: any) => string> {
    ...
    if (fields) {
      const encodedType = await poseidonId(this.#fullTypes.get(type) as string);

      // Asynchronous processing is completed in advance and stored in variables
      const fieldEncoders = await Promise.all(
        fields.map(async ({type}) => await this.getEncoder(type)),
      );
   ...
   */
  async encodeData(type: string, value: any): Promise<string> {
   ...
   */
  async hashStruct(name: string, value: Record<string, any>): Promise<string> {
    return await hashMessage(await this.encodeData(name, value));
  }
   ...
   */
  async hash(value: Record<string, any>): Promise<string> {
   ...
   */
  static async hashDomain(domain: TypedDataDomain): Promise<string> {
    ...
    const domainHash = await TypedDataEncoder.hashStruct(
      'EIP712Domain',
  ...
  static async hash(
    ...
    return await hashMessage(
      await TypedDataEncoder.encode(domain, types, value),
    );
  }
...
export async function verifyTypedData(
  ...
  return recoverAddress(
    await TypedDataEncoder.hash(domain, types, value),
    signature,
  );
}
