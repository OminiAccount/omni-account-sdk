...
import { getAddress, keccak256, recoverAddress, concat, defineProperties, getBigInt, getBytes, hexlify, isHexString, mask, toBeHex, toQuantity, toTwos, zeroPadValue, assertArgument, id, } from 'ethers';
...
export class TypedDataEncoder {
    ...
    async #getEncoder(type) {
        ...
        if (fields) {
            const encodedType = await poseidonId(this.#fullTypes.get(type));
    ...
    static async hash(domain, types, value) {
        return await hashMessage(await TypedDataEncoder.encode(domain, types, value));
    }
 ...
 */
export async function verifyTypedData(domain, types, value, signature) {
    return recoverAddress(await TypedDataEncoder.hash(domain, types, value), signature);
}
